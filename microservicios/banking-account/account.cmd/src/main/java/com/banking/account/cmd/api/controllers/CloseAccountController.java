    ...
    public ResponseEntity<BaseResponse> closeAccount(@PathVariable(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(
                    new BaseResponse("Se cerró la cuenta bancaria exitosamente"),
                    HttpStatus.OK
            );
        } catch (IllegalStateException | AggregateNotFoundException e) {
            logger.log(Level.WARNING, MessageFormat.format("El cliente envío un request con errores {0} ", e.toString()));
            return new ResponseEntity<>(
            ...
            logger.log(Level.WARNING, MessageFormat.format("Error {0} ", e.toString()));
