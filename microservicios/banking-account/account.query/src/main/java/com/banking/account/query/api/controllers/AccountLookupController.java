...
import com.banking.account.query.api.queries.FindAcountByIdQuery;
import com.banking.account.query.api.queries.FindAllAcountsQuery;
...
public class AccountLookupController {
    ...
    public ResponseEntity<AccountLookupResponse> getAllAccounts() {
        try {
            List<BankAccount> accounts = queryDispatcher.send(new FindAllAcountsQuery());
            ...
            var response = AccountLookupResponse
                    .builder()
                    .accounts(accounts)
                    .message(MessageFormat.format("Se realizó la consulta con exito", null))
                    .build();
    ...
    public ResponseEntity<AccountLookupResponse> getAccountById(@PathVariable(value = "id") String id) {
        try {
            List<BankAccount> accounts = queryDispatcher.send(new FindAcountByIdQuery(id));
            if (accounts == null || accounts.size() == 0) {
                return new ResponseEntity<>(null, HttpStatus.NO_CONTENT);
            }
    ...
    public ResponseEntity<AccountLookupResponse> getAccountByWithBalance(
